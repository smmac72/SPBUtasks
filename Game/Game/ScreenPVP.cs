public class ScreenPVP : ScreenCore
{
    ...
    override public void ScreenUpdate(string[] Field, Call CurrentCall, int CurrentTime)
    {
        ...
        { 
            ...
            if (InternalCallCode != -1)
            {
                CurrentCall = InternalCallCreator.InitiateCall(InternalCallCode);
                InternalCallCode = -1;
            }
    ...
    void PrintChat(Call CurrentCall)
    {
        ...
        if (CurrentChat != null)
        {
            ...
            Console.WriteLine("Trainer Hint: SET *number* - SET DESIRED SITUATION");
    ...
    bool IsAcceptableAnswer(string answer)
    {
        if (answer == null)
            return false;
        answer = answer.Split(' ')[0]; // костыль на кооп
        string[] AcceptableAnswers = {
            "NAME", "LOC", "SIT",
            "POLICE", "EMS", "FIRE", "END", "SET" };
    ...
    void ProcessInput(Call CurrentCall, ChatBox CurrentChat)
    {
        ...
        if (IsAcceptableAnswer(InputCommand))
        {
            ...
            if (InputCommand.Split(' ')[0].Equals("SET") && InputCommand.Split(' ').Length > 1)
            {
                int NewCallID = -1;
                if (Int32.TryParse(InputCommand.Split(' ')[1], out NewCallID))
                    if (NewCallID >= 0 && NewCallID <= 15)
                        InternalCallCode = NewCallID;
            }
        }
    }
}
